...
#Connecting to the device via SSH,  delay of 3 seconds..
...
#Defining the file from yesterday, for comparison.  isoformat:2020-04-27
...
##Save the diff html files.
#diff_file = open("diff.html", "w")
#diff_file.write(difference)
#diff_file.close()

diff_file = open('cfgfiles/' 'DIFF' '_' + ip + '_' + datetime.date.today().isoformat() + '.html', 'w')
diff_file.write(difference)

#Sending the differences via email
#Defining the e-mail parameters
#fromaddr = 'mc.malinao@gmail.com'
#toaddr = 'mc.malinao@gmail.com'
...
#Logging in to Gmail and sending the e-mail sendmail requires to send as string..
#server.login('mc.malinao', 'J0@nn1230')
#server.quit()
